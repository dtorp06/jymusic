...
class AdsController extends AddonsController{
	...
	public function updateseat(){
		...
		if(empty($data['id'])){ //新增数据
			...
			if(!$id){
				...
				$this->success('新增成功', U('/Addons/adminList/name/Ads/seat'));				
			}
		} else { //更新数据
			...
			if(false === $status){
				...
				$this->success('更新成功', U('/Addons/adminList/name/Ads/seat'));
			}
		}
	}
	...
	public function seatstatus(){
		...
		if($status == 1){
			...
			$this->success('成功启用该广告位',U('/Addons/adminList/name/Ads/seat'));
		}else{
			foreach ($ids as $id){
