...
class HttpDown {  
	...
	**/  
	function initialize() {  
	   ...
	   if ($this->use_auth) {//使用身份验证
			if (!$this->_auth()){ //验证没有通过 
	   ...
	   if ($this->mime == null) $this->mime = "application/octet-stream"; //默认 mime  
	...
	**/  
	function download() {  
		...
		if ($this->data_type == 0){  //文件下载
			...
			$this->header($size,$seek,$this->seek_end); //总是使用最后的寻求 
