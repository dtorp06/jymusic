		...
		<div class="row clearfix">
			<div class="col-sm-8 col-xs-12">
				<div class="clearfix mb_15">
						<h2 class="f_left pt_10">全部艺人</h2>
						<div class="custom_select relative portfolio_filter clearfix f_right">
							...
							<select>
								...
								<?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option data-filter=".type_<?php echo ($vo["id"]); ?>" value="type_<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
							</select>
						</div>
				</div>
				<hr class="mb_10 ">
				...
				<section class="portfolio_masonry_container">
					<?php $where=array();$where["status"]=1;$_result = M("Artist")->alias("__MUSIC")->where($where)->page(!empty($_GET["p"])?$_GET["p"]:1,28)->order("update_time desc")->select();if($_result):$i=0;foreach($_result as $key=>$vo): $vo['description']=text_filter($vo['introduce']); $vo['url']=U('/artist/'.$vo['id']); $vo['cover_url']= !empty($vo['cover_url'])? $vo['cover_url'] : "/Public/static/images/artist_cover.png";++$i;$mod = ($i % 2 );?><div class="portfolio_item t_align_c type_<?php echo ($vo["type_id"]); ?>">
						<figure class="d_inline_b ">
							<div class="photoframe  relative shadow wrapper artist_cover">
								...
								<figcaption class="t_align_c text-ellipsis">
									<h5 class="mb_3 mt_5"><?php echo ($vo['name']); ?></h5>
								</figcaption>
							</div>
						</figure>
					</div><?php endforeach; endif; $vo_total= M("Artist")->where($where)->count();$__PAGE__ = new \Think\Page($vo_total,28);$__PAGE__ ->rollPage = 5;$__PAGE__->setConfig("theme","%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%");$__PAGE__->setConfig("prev", "上页");$__PAGE__->setConfig("next", "下页");$vo_page= $__PAGE__->show(); ?>
				</section>
				...
				<div class="clearfix mb_15">
					...
					<div class="col-sm-8 col-xs-7 t_align_r">
			...
			<div class="col-sm-4 col-xs-12">
				<!--侧边栏-->
				<h2 class="pt_10">新晋艺人<a href="<?php echo U('Artist/index');?>" class="">/更多</a></h2>
<hr>
<div class="sidebar_content">
	<?php $where=array();$where["status"]=1;$_result = M("Artist")->alias("__MUSIC")->where($where)->cache(true,86400)->limit("6")->order("add_time desc")->select();if($_result):$i=0;foreach($_result as $key=>$v): $v['description']=text_filter($v['introduce']); $v['url']=U('/artist/'.$v['id']); $v['cover_url']= !empty($v['cover_url'])? $v['cover_url'] : "/Public/static/images/artist_cover.png";++$i;$mod = ($i % 2 );?><div class="clearfix ">
		<img src="<?php echo ($v['cover_url']); ?>" alt="" class="f_left circle mr_15" height="50" width="50">
		<a href="<?php echo ($v['url']); ?>" class="d_block "><?php echo ($v['name']); ?></a>
		<p class="scheme_color">
			<span class="f_left"><i class="jy jy-fire"></i> <?php echo ($v['hits']); ?></span>
			<a href="<?php echo ($v['url']); ?>" class=" button_type_19  f_right tr_delay_hover  bg_dark_color">详细</a>					
		</p>		
	</div>
	<hr class="mb_15"><?php endforeach; endif;?>
</div>	
