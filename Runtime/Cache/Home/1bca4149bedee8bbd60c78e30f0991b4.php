		...
		<div class="row clearfix">
			...
			<section class="col-sm-8 pt_10">
				...
				<section class="list_type clearfix mb_20">						
					<?php $_result = M("Tag")->alias("__TAG")->where(array("status"=>1))->limit(20)->order("count desc")->select();if($_result):$i=0;foreach($_result as $key=>$v): $v['url']= !empty($v["alias"])? U('/tag/'.$v['alias']): U('/Tag/'.$v['id']);++$i;$mod = ($i % 2 );?><a class="tr_delay_hover btn_1 f_left mr_5 mt_5" target="_tag" href="<?php echo U('tag/'.$v['alias']);?>"><?php echo ($v['name']); ?></a><?php endforeach; endif;?>
				</section>		
				
				<?php $_result = get_tag_tree();if($_result):$i=0;foreach($_result as $key=>$tag): $tag['url']= !empty($tag["alias"])? U('/tag/'.$tag['alias']): U('/Tag/'.$tag['id']);++$i;$mod = ($i % 2 ); if(!empty($tag['tags'])): ?><div class="row clearfix pt_10">					
						<div class="col-lg-5 col-md-4 col-sm-4 col-xs-5 ">
							<h3 class=""><?php echo ($tag["tag_group"]); ?></h3>
						</div>
					</div>
					<hr class="mb_20 ">
					<section class="list_type clearfix mb_20">						
						<?php if(is_array($tag['tags'])): $i = 0; $__LIST__ = $tag['tags'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><a href="<?php echo U('tag/'.$t['alias']);?>" target="_tag" class="btn_1 f_left mr_5 mt_5"><?php echo ($t['name']); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
