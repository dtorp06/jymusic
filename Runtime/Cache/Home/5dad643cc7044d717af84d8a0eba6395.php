			...
			<section class="col-sm-8 pt_10">				
				<div class="row clearfix">					
					<div class="col-sm-4 col-xs-5 t_align_l">
						<h2 class="pt_10"><i class="jy jy-cdcover mr_10"></i><?php echo ($title); ?></h2>
					</div>
					<div class="col-sm-8 col-xs-7 t_align_l ">
						<div class="clearfix ml_10">
							<div class="custom_select relative f_right t_align_l">
								<div class="select_title  relative color_dark"><?php echo ($title); ?> </div>
								<ul class="select_list d_none">
									<li class="tr_delay_hover"><a href="<?php echo U('/Album');?>">全部专辑</a></li>
									<?php $where=array();$_result = M("AlbumType")->alias("__MUSIC")->where($where)->cache(true,72000)->field("id,name,description")->select();if($_result):$i=0;foreach($_result as $key=>$vo): $vo['url']=U('/album/type-'.$vo['id']);++$i;$mod = ($i % 2 );?><li class="tr_delay_hover"><a href="<?php echo ($vo['url']); ?>"><?php echo ($vo["name"]); ?></a></li><?php endforeach; endif;?>
								</ul>
							</div>
						</div>
					</div>
				...
				<?php $where=array();$where["status"]=1;$_result = M("Album")->alias("__MUSIC")->where($where)->page(!empty($_GET["p"])?$_GET["p"]:1,12)->order("update_time desc")->field("*")->select();if($_result):$i=0;foreach($_result as $key=>$vo): $vo['description']=text_filter($vo['introduce']); $vo['url']=U('album/'.$vo['id']); $vo['artist_url']=U('/artist/'.$vo['artist_id']); $vo['user_url']=U('/user/'.$vo['add_uid']); $vo['genre_url']=U('/genre/'.$vo['genre_id']); $vo['type_url']=U('/album/type-/'.$vo['type_id']); $vo['cover_url']= !empty($vo['cover_url'])? $vo['cover_url'] : "/Public/static/images/album_cover.png";++$i;$mod = ($i % 2 );?><div class="row clearfix a_list">
					...
					<div class="col-sm-6 col-xs-8">
						...
						<p class="scheme_color mb_15">
							发行时间：
							<?php if(empty($vo['pub_time'])): ?>未知
							<?php else: ?>
								<?php echo ($vo['pub_time']); endif; ?> 
							唱片公司：
							<?php if(empty($vo['company'])): ?>暂无
							<?php else: ?>
								<?php echo ($vo['company']); endif; ?> 
						</p>
						<p class="a_list_info"><?php echo (msubstr($vo["description"],0,92)); ?> <a href="<?php echo ($vo['url']); ?>" class="ml_10" target="_album">【详细】</a></p>
					</div>
...
<?php echo hook('Ads','right_up');?>
			...
			<h2 class="pt_10">音乐人<a href="<?php echo U('Artist/index');?>" class="">/更多</a></h2>
<hr>
<div class="sidebar_content">
	<?php $where=array();$where["status"]=1;$_result = M("Artist")->alias("__MUSIC")->where($where)->cache(true,86400)->limit("10")->order("hits desc")->select();if($_result):$i=0;foreach($_result as $key=>$v): $v['description']=text_filter($v['introduce']); $v['url']=U('/artist/'.$v['id']); $v['cover_url']= !empty($v['cover_url'])? $v['cover_url'] : "/Public/static/images/artist_cover.png";++$i;$mod = ($i % 2 );?><div class="clearfix ">
