	...
	<div class="container">	
		<div class="row clearfix">
			<aside class="col-sm-3">
				<h2 class="pt_10">榜单排行(TOP100)</h2>
				<hr>	
				<div class="sidebar_content">
					<ul class="main_menu c_rank">										
						<li class="<?php if(($order) == "listens"): ?>current<?php endif; ?>">
							<a class="" href="<?php echo U('/Ranks');?>">人气排行</a>
						</li>
						<li class="<?php if(($order) == "likes"): ?>current<?php endif; ?>">
							<a class="" href="<?php echo U('Ranks/hot');?>">最新推荐</a>
						</li>

						<li class="<?php if(($order) == "download"): ?>current<?php endif; ?>">
							<a class="" href="<?php echo U('Ranks/down');?>">下载排行</a>
						</li>
						<li class="<?php if(($order) == "favtimes"): ?>current<?php endif; ?>">
							<a class="" href="<?php echo U('Ranks/fav');?>">收藏排行</a>
						</li>
						<li class=" <?php if(($order) == "2"): ?>current<?php endif; ?>">
							<a class="" href="<?php echo U('Ranks/latest');?>">最新上传</a>
						</li>						
					</ul>
				</div>					
			</aside>
			...
			<section class="col-sm-9">
				...
				<div class="clearfix mt_10 ">
					<?php $where=array();$where["status"]=1;$_result = M("Songs")->alias("__MUSIC")->where($where)->page(!empty($_GET["p"])?$_GET["p"]:1,20)->order($order." desc")->select();if($_result):$i=0;foreach($_result as $key=>$lv): $lv['url']=U('/music/'.$lv['id']); $lv['down_url']=U('/down/'.$lv['id']); $lv['user_url']=U('/user/'.$lv['up_uid']); $lv['artist_url']=U('/artist/'.$lv['artist_id']); $lv['album_url']=U('/album/'.$lv['album_id']); $lv['genre_url']=!empty($lv['genre_id']) ? U('/genre/'.$lv['genre_id']) : U('/Genre'); $lv['cover_url']= !empty($lv['cover_url'])? $lv['cover_url'] : "/Public/static/images/cover.png";++$i;$mod = ($i % 2 );?><div class="mb_10 clearfix relative play_box">
						...
						<hr class="l_hr">
					</div><?php endforeach; endif; $lv_total= 100;$__PAGE__ = new \Think\Page($lv_total,20);$__PAGE__ ->rollPage = 5;$__PAGE__->setConfig("theme","%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%");$__PAGE__->setConfig("prev", "上页");$__PAGE__->setConfig("next", "下页");$lv_page= $__PAGE__->show(); ?>
				</div>
