       			...
       			<div class="row">
	       			...
	       			<div class="col-lg-6">
	       				<div class="btn-group pull-right">
	        				<a id="add-group" class="btn btn-labeled btn-success" href="<?php echo U('createGroup');?>">新增</a>
	        				<a url="<?php echo U('changestatus',array('method'=>'deleteGroup'));?>" class="btn btn-labeled btn-danger ajax-post confirm" target-form="ids">删除</a>
	        				<a url="<?php echo U('changestatus',array('method'=>'resumeGroup'));?>" class="btn btn-labeled btn-success ajax-post" target-form="ids">启用</a>
	        				<a url="<?php echo U('changestatus',array('method'=>'forbidGroup'));?>" class="btn btn-labeled btn-danger ajax-post" target-form="ids">禁用</a>
		         		</div>
		         	</div>
	         	</div>      			
       		</div>
    		<div class="table-responsive">
				...
				<div id="admin" class="tab-pane fade active in">
	    			<table class="table table-striped table-bordered table-hover"> 
	            		...
	            		<tbody>
	        			<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["module"] == 'admin' ): ?><tr>
	                    	<td>
	        				...
	        				<td>
	        					<a href="<?php echo U('AuthManager/access?group_name='.$vo['title'].'&group_id='.$vo['id']);?>" >访问授权</a>
	        				...
	        				<td class='text-center'><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('AuthManager/changeStatus?method=forbidGroup&id='.$vo['id']);?>" class="btn btn-labeled btn-sm btn-danger ajax-get">禁用</a>
	        					<?php else: ?>
	        					<a href="<?php echo U('AuthManager/changeStatus?method=resumeGroup&id='.$vo['id']);?>" class="btn btn-labeled btn-sm btn-success ajax-get">启用</a><?php endif; ?>
	        					<a href="<?php echo U('AuthManager/editgroup?id='.$vo['id']);?>" class="btn btn-labeled btn-sm">编辑</a>
	        					<a href="<?php echo U('AuthManager/changeStatus?method=deleteGroup'.$vo['id']);?>" class="btn btn-labeled btn-sm btn-danger confirm ajax-get">删除</a>
	                        </td>
				...
				<div id="home" class="tab-pane fade">
	    			<table class="table table-striped table-bordered table-hover"> 
	            		...
	            		<tbody>
	        			<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["module"] == 'home' ): ?><tr>
	                    	<td>
	                        	<div class="checkbox c-checkbox">
	                        		<label>
	                            		<input type="checkbox" class="ids" value="<{vo.id}>" name="id[]">
...
highlight_subnav("<?php echo U('User/index');?>","<?php echo U('AuthManager/index');?>");
</script>
