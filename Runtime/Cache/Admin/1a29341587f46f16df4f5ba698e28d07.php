    	...
    	<div class="panel panel-default">
       		<div class="panel-heading ">导航列表
       			<div class="btn-group pull-right">
	         		<a class="btn btn-labeled btn-success " href="<?php echo U('add');?>">新 增</a>
					<a class="btn btn-labeled btn-danger" href="javascript:;">删 除</a>
					<button class="btn btn-labeled btn-success list_sort" url="<?php echo U('sort',array('pid'=>I('get.pid',0)),'');?>">排序</button>
                    	...
                    	<?php if(!empty($list)): if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($k % 2 );++$k;?><tr>
                        	<td>
                          		<div class="checkbox c-checkbox">
                             		<label>
                                	<input class="ids row-selected"  type="checkbox" value="<?php echo ($data["id"]); ?>" name="id[]">
                                	<span class="fa fa-check"></span>
                             		</label>
                          		</div>
                       		</td>			
        					<td><?php echo ($data["id"]); ?></td>
        					<td class='m-name'><?php echo ($data["title"]); ?></td>
        					<td><?php echo (get_channel_name($data["pid"])); ?></td>
        					<td><?php echo ($data["url"]); ?></td>
        					<td><?php echo ($data["sort"]); ?></td>
                        	<td><?php echo (time_format($data["create_time"],'Y-m-d')); ?></td>
                        	<td class='text-center'>
                        		<a class="btn btn-labeled btn-sm " href="<?php echo U('edit?id='.$data['id']);?>"><em class="fa fa-edit"></em></a>
	                            <a class="btn btn-labeled btn-sm " href="<?php echo U('del?id='.$data['id']);?>"><em class="fa fa-times"></em></a>
                        	</td>
...
<script type="text/javascript">
		highlight_subnav("<?php echo U('Config/group');?>","<?php echo U('Channel/index');?>");
	    $(function() {
    	//点击排序
