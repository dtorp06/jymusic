...
<div class="col-md-4">
	<div class="input-group mb-lg search-form">
    	<input type="text" class="form-control search-input" name="title" value="" placeholder="请输入专辑名称">
    	<a class="input-group-addon btn" id="search" url="<?php echo U('Album/index','pid='.I('pid',0),false);?>">
        		...
        		<table class="table table-striped table-bordered table-hover">           
                    ...
                    <tbody role="alert" aria-live="polite" aria-relevant="all">
                    	<?php if(!empty($list)): if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($k % 2 );++$k;?><tr>
                        	...
                        	<td class='m-name'><a class="edit-name" href="#"><?php echo ($data["name"]); ?></a></td>
                        	...
                        	<td><?php echo ($data["rater"]); ?></td>
        					<td><?php echo (get_status_title($data["status"])); ?></td>
                        	<td><?php echo (time_format($data["add_time"],'Y-m-d')); ?></td>
                    		<td class="text-center">
	                            <a class="btn btn-labeled btn-sm mr-sm" href="<?php echo U('mod?id='.$data['id']);?>"><em class="fa fa-edit"></em></a>
	                            <a class="btn btn-labeled btn-sm btn-danger btn-del" url="<?php echo U('del?id='.$data['id']);?>"><em class="fa fa-times"></em></a>
                         	</td>
                      	</tr><?php endforeach; endif; else: echo "" ;endif; ?>
        				<?php else: ?>
        				<td colspan="13" class="text-center">暂时还没有内容! </td><?php endif; ?>
                    </tbody>
                </table>
    		</div>
    		<!-- 结束 表格 -->
...
<script type="text/javascript">
	//导航高亮
	highlight_subnav("<?php echo U('Songs/index');?>","<?php echo U('Album/index');?>")
