...
<script type="text/javascript" src="/Public/Admin/js/miniplay/swfobject.js"></script>
<script type="text/javascript">
	var so = new SWFObject("/Public/Admin/js/miniplay/audioplayer.swf","player","220","20","9","#eef0f4");
        		...
        		<table class="table table-striped table-bordered table-hover">           
        			...
        			<tbody role="alert" aria-live="polite" aria-relevant="all">
        				<?php if(!empty($list)): if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($k % 2 );++$k;?><tr>           
                        	...
                        	<td><?php echo ($data["up_uname"]); ?></td>
                        	<td><?php echo ($data["album_name"]); ?></td>
                        	<td><?php echo ($data["genre_name"]); ?></td>
                        	<td><?php echo ($data["artist_name"]); ?></td>
                        	<th>
								<a data-id="<?php echo ($data["id"]); ?>" herf="javascript:;" class="btn btn-labeled btn-sm btn-listen">试听</a>					
							</th>
                        	<td><?php echo (get_status_title($data["status"])); ?></td>
                        	<td><?php echo (time_format($data["add_time"],'Y-m-d')); ?></td>
                    		<td class="text-center">
	                            <a class="btn btn-labeled btn-sm btn-sh" herf="javascript:;" url="<?php echo U('pass');?>" data-id="<?php echo ($data['id']); ?>" data-uid="<?php echo ($data['up_uid']); ?>">通过</a>
	                            <a class="btn btn-labeled btn-sm btn-sh" herf="javascript:;" url="<?php echo U('disable');?>"  data-id="<?php echo ($data['id']); ?>" data-uid="<?php echo ($data['up_uid']); ?>">驳回</a>
	                            <a class="btn btn-labeled btn-sm btn-danger btn-del" herf="javascript:;" url="<?php echo U('Songs/setStatus',array('status'=>-1,'ids'=>$data['id']));?>">删除</a>
                         	</td>
...
$(function(){
	...
	$('.btn-listen').click(function () {
		...
		$.ajax({ 
			type: "POST",
			url:"<?php echo U('listen');?>", 
			data:{id:id},
			success: function(data){
				var player = $('<audio   src="'+data.music_url+'" controls="controls"></audio>');
				that.hide().before(player);
				$("audio").each(function(){
					this.pause();
				});
				player[0].volume=0.8;//表示的是播放音量为一半
				player[0].play();
			},
