		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<ul class="nav nav-pills">
					...
					<li><a href="<?php echo U('view');?>">视图配置</a></li>
				</ul>
			</div>
		
			<div class="panel-body">
				<form class="form-horizontal" action="<?php echo U('save');?>"  method="post">
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><div class="form-group">											
					...
					<?php switch($config["type"]): case "0": ?><div class="col-sm-2">
						<input type="text" class="form-control" name="config[<?php echo ($config["name"]); ?>]" value="<?php echo ($config["value"]); ?>">
					</div><?php break;?>
					<?php case "1": ?><div class="col-sm-4">
						<input type="text" class="form-control" name="config[<?php echo ($config["name"]); ?>]" value="<?php echo ($config["value"]); ?>">
					</div><?php break;?>
					<?php case "2": ?><div class="col-sm-5">
						<textarea class="form-control" name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config["value"]); ?></textarea>
					</div><?php break;?>
					<?php case "3": ?><div class="col-sm-3">
						<textarea class="form-control" name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config["value"]); ?></textarea>
					</div><?php break;?>
					<?php case "4": ?><div class="col-sm-3">
						<select name="config[<?php echo ($config["name"]); ?>]" class="form-control" style= "width:auto">
						<?php $_result=parse_config_attr($config['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if(($config["value"]) == $key): ?>selected="selected"<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</div><?php break; endswitch;?>					
					<span class="help-inline"><?php echo ($config["remark"]); ?></span>			
