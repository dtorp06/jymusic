    ...
    <div class="col-lg-12">
    	<div class="panel panel-default">
			...
			<div class="btn-group ml-lg">
       				[<?php if(I('status')): ?><a href="<?php echo U('index');?>" class="mr-sm ml-sm"> 全部</a>
						<a href="<?php echo U('index?status=1');?>" class="mr-sm ml-sm <?php if(I('status') == 1): ?>text-danger<?php endif; ?>">已审核</a>
						<a href="<?php echo U('index?status=2');?>" class="ml-sm mr-sm <?php if(I('status') == 2): ?>text-danger<?php endif; ?>">待审核</a>
					<?php else: ?>
						<a href="<?php echo U('index');?>" class="mr-sm ml-sm text-danger"> 全部</a>
						<a href="<?php echo U('index?status=1');?>" class="mr-sm ml-sm ">已审核</a>
						<a href="<?php echo U('index?status=2');?>" class="ml-sm mr-sm ">待审核</a><?php endif; ?>]
       			</div>
       			<div class="btn-group pull-right">
	       			<!--a class="btn btn-labeled btn-success ajax-post "  href="<?php echo U('setStatus',array('status'=>1));?>" target-form="ids">批量审核</a--> 
	       			<!--a class="btn btn-labeled btn-success ajax-post "  href="<?php echo U('disable');?>" target-form="ids">未通过</a-->             
        		...
        		<table class="table table-striped table-bordered table-hover">           
        			...
        			<tbody role="alert" aria-live="polite" aria-relevant="all">
        				<?php if(!empty($list)): if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($k % 2 );++$k;?><tr>           
                        	...
                        	<td class='m-name'><?php echo ($data["realname"]); ?></td>
                        	<td><?php echo ($data["idcard"]); ?></td>
                        	<td><?php echo ($data["phone"]); ?></td>
                        	<td>
								<a  herf="javascript:;" url="<?php echo (get_cover($data['attach_id'])); ?>" class="btn btn-labeled btn-sm btn-view">查看</a>					
							</td>
                        	<td><?php echo ($data["reason"]); ?></td>
							<td><?php echo (get_status_title($data["status"])); ?></td>
                        	<td><?php echo (time_format($data["create_time"],'Y-m-d')); ?></td>
                    		<td class="text-center">
								<?php if(($data['status']) == "2"): ?><a class="btn btn-labeled btn-sm ajax-get" href="<?php echo U('pass?uid='.$data['uid']);?>">通过</a>
	                            <a class="btn btn-labeled btn-sm btn-sh" href="<?php echo U('disable?id='.$data['id']);?>" >驳回</a>
								<?php else: ?>
	                            <a class="btn btn-labeled btn-sm btn-danger btn-del" herf="javascript:;" url="<?php echo U('del',array('id'=>$data['id']));?>">取消</a><?php endif; ?>
							</td>
...
<script type="text/javascript">
//导航高亮
highlight_subnav("<?php echo U('Audit/index');?>");
