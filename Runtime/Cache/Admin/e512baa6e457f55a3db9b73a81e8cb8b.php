<?php if (!defined('THINK_PATH')) exit(); if($param['type'] == 'img'): ?><script type="text/javascript" src="/Public/static/webuploader/webuploader.min.js"></script>
...
$(function($){
	var uploader = WebUploader.create({
		...
		// 文件接收服务端。
		server	: '<?php echo U('File/uploadPicture',array('session_id'=>session_id(),'type'=>$dtype));?>',
...
<?php else: ?>
	<?php if(($config['up_on']) == "1"): if($config['up_type'] == 1): ?><!-- ftp-->
		
		<?php elseif($config['up_type'] == 2): ?>
		<script type="text/javascript">
		var  url = 'http://<?php echo ($config['alioss_host']); ?>';
		$(document).ready(function(){
			var obj = $.ajax({
				url: "<?php echo addons_url('Justup://Call/alioss');?>",
				async: false,
				dataType:'json'
			}).responseJSON;
		...
		$(document).ready(function(){			
			...
			uploadListen.bind('BeforeUpload', function(up, file) {
				...
				//显示要上传的服务器
				$.ajax({
					url: "<?php echo addons_url('Justup://Call/qiniuServer');?>",
					type: "POST",
		...
		function getSin (file){
			return $.ajax({
				url: "<?php echo addons_url('Justup://Call/qiniu');?>",
				type: "POST",
				data:{file:file['name']},
				async: false,
			}).responseText;
	...
	<!--本地上传-->
	<script type="text/javascript">
		var  url = '<?php echo U('File/uploadMusic',array('session_id'=>session_id()));?>';		
		$(document).ready(function(){
			uploadListen.bind('FileUploaded', function(up, file,info) {
				var btn 	= uploadListen.getOption('browse_button');
				uploaded( btn,info);				
	...
	<script type="text/javascript" src="/Public/static/plupload/zh_CN.js"></script>
	<script type="text/javascript">
	var options = {
