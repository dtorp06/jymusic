...
<?php $where=array();$where["status"]=1;$_result = M("Album")->alias("__MUSIC")->where($where)->limit("10")->order("add_time desc")->field("*")->select();if($_result):$i=0;foreach($_result as $key=>$v): $v['description']=text_filter($v['introduce']); $v['url']=U('album/'.$v['id']); $v['artist_url']=U('/artist/'.$v['artist_id']); $v['user_url']=U('/user/'.$v['add_uid']); $v['genre_url']=U('/genre/'.$v['genre_id']); $v['type_url']=U('/album/type-/'.$v['type_id']); $v['cover_url']= !empty($v['cover_url'])? $v['cover_url'] : "/Public/static/images/album_cover.png";++$i;$mod = ($i % 2 );?><div class="tm_div al_ls">
	<div class="tm_pro">
		<a href="<?php echo ($v['url']); ?>"><img class="tm_img"  src="<?php echo ($v['cover_url']); ?>"></a>
		<div class="tm_title">
			<div class="tm_title_bg"></div>
			<div class="tm_title_con">
				<div class="tm_title_con_1"><a href="<?php echo ($v['url']); ?>"><?php echo ($v['name']); ?></a></div>
			</div>
		</div>
	</div>
	<div class="tm_price">
		<a href="<?php echo ($v['artist_url']); ?>"><?php echo ($v['artist_name']); ?></a>
	</div>
</div><?php endforeach; endif;?>
<div id="more" class="more" url="<?php echo U('getlist');?>">点击查看更多</div>
