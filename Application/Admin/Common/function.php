...
function get_status_title($status = null){
    ...
    switch ($status){
        ...
        case -1 : return    '已删';   break;
        case 0  : return    '禁用';     break;
        case 1  : return    '正常';     break;
        case 2  : return    '待审';   break;        
...
function get_best_cate($cid){
	$pid	= D('Category')->getFieldById($cid,'pid');
	if ($pid != 0){
		$cid = get_best_cate($pid);
	}
	return $cid;	
}
...
function get_custom_config($name) {
	$confingPath ='./Data/Conf/'.$name.'.php';
...
/*获取设置随机数*/
function setrand ($str) {
	if(!empty($str)){
		$str = @explode("-", $str);
		if(count($str) > 1){
			return  rand(trim($str[0]),trim($str[1])); 
		}else{
			return  trim($str[0]);											
			//return rand(trim($str[0]),trim($str[1]);
		}
...
/*获取设置随机数*/
function get_str_sort ($str) {
	//获取首个字符\
	$fristStr 	= msubstr( $str, 0, 1,"utf-8", false );

	if (preg_match("/^[\x7f-\xff]+$/", $fristStr)){ 
		//汉字获取拼音
		import('JYmusic.PinYin');
		$pinYin = new \PinYin();
		$pinyin = $pinYin->Pinyin($fristStr);
		$sort = substr( $pinyin, 0, 1 ); 
	}else { 
		$sort = $fristStr; 
	}
	return $sort;
...
function __basename($path){
    $path=str_replace("\\","/",$path);
    $path=explode("/",$path);
    return file_name_convert($path[count($path)-1]);
}

//文件名转码
function file_name_convert($str){
 ...
 * 文件下载到服务器
 */
function file_download ($from, $file_name){
	$fp = @fopen ($from, "rb");
	if ($fp){
...
function check_update(){
	...
	$model	= M('Config');		
	$uptime		= $model->where(array('name' =>'JYMUSIC_UPDATE_TIME'))->getField('value');
	$version	= $model->where(array('name' =>'JYMUSIC_VERSION'))->getField('value');
	$params = array(
		'version'		=> $version,
		'updateTime' 	=> $uptime,
