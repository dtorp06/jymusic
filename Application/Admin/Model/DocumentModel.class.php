...
class DocumentModel extends Model{
    ...
    protected $_auto = array(
		...
		array('description', 'getDec', self::MODEL_BOTH, 'callback'),
        array('root', 'getRoot', self::MODEL_BOTH, 'callback'),
		array('cover_id', 'getCover', self::MODEL_BOTH, 'callback'),
    ...
    function getCover ($cid){
		...
		if (!empty($array[1])){
			$data['url']		= $array[1][0];
			$model	= M('Picture');
			$id = $model->where($data)->getField('id');
